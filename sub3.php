...
$query="SELECT totalclass FROM $sub3";
...
if(isset($_POST['submit'])){//echo "string";
  ...
  $stmt="INSERT INTO sub3presentabsent(date,totalpresent,totalabsent) VALUES('$date','$tpresent','$tabsent');";
   $conn->exec($stmt);
...
//var_dump($tclass);
  ...
  for ($i=0; $i < count($tclass); $i++) { 
    $j=$i;
    $j+=1;
    $update=$conn->prepare("UPDATE $sub3 SET totalclass=$tclass[$i] where rollnumber=$j");
    $update->execute();
  }

  $sub3tclass+=1;
  $update=$conn->prepare("UPDATE subjects SET totalclass='$sub3tclass' where subname='$sub3'");
  $update->execute();
