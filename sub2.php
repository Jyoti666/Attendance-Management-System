...
$query="SELECT totalclass FROM $sub2";
...
if(isset($_POST['submit'])){//echo "string";
  ...
  $stmt="INSERT INTO sub2presentabsent(date,totalpresent,totalabsent) VALUES('$date','$tpresent','$tabsent');";
   $conn->exec($stmt);
...
//var_dump($tclass);
  ...
  for ($i=0; $i < count($tclass); $i++) { 
    $j=$i;
    $j+=1;
    $update=$conn->prepare("UPDATE $sub2 SET totalclass=$tclass[$i] where rollnumber=$j");
    $update->execute();
  }
  
  $sub2tclass+=1;
  $update=$conn->prepare("UPDATE subjects SET totalclass='$sub2tclass' where subname='$sub2'");
  $update->execute();
